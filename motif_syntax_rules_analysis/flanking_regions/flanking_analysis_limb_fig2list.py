...
if True:
    import numpy as np
    inputmtx_out = np.load("/groups/stark/ken.murakami/mouse_enhancer/input_limbmodel.npy")
    shapoh_out   = np.load("/groups/stark/ken.murakami/mouse_enhancer/shap_limbmodel.npy")
...
if True:
    ...
    outdir    = "/groups/stark/ken.murakami/mouse_enhancer/flank_limb_fig2list_ver7"
    ...
    for motif_name in motiflist:
        ...
        try:
            fig, ax, pvals, final_idx, final_labels = plot_selected_positions_boxplot_edge_based(
                ...
                p_threshold=0.05,
                figsize=(12, 5.8),
                title="%s flanking contribution for limb model" % motif_name,
                pdf_path="%s/%s_flank_edge_based_top1_bottom1_limb_fig2list.pdf" % (outdir, motif_name_safe),
                ylim=None,
                high_label="Top 1%",
                low_label="Bottom 1%",
            )
        ...
        try:
            ...
            result = plot_fig4b_like(
                ...
                figsize=(10, 8),
                title="%s flanking regions for limb model" % motif_name,
                pdf_path="%s/fig4b_like_%s_top_sorted_limb_fig2list.pdf" % (outdir, motif_name_safe),
                box_ylim=None,
                effect_norm="std",
