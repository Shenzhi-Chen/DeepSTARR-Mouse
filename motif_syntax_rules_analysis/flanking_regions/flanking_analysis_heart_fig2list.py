...
if True:
    import numpy as np
    inputmtx_out = np.load("/groups/stark/ken.murakami/mouse_enhancer/input_heartmodel.npy")
    shapoh_out   = np.load("/groups/stark/ken.murakami/mouse_enhancer/shap_heartmodel.npy")
...
if True:
    ...
    outdir    = "/groups/stark/ken.murakami/mouse_enhancer/flank_heart_fig2list_ver7"
...
if True:
    ...
    def pwm_to_consensus(pwm, threshold=0.6, trim_terminal_n=True):
        consensus_chars = []
...
if True:
    ...
    for motif_name in motiflist:
        ...
        print("Cluster         :", clusters[motif_name])
        ...
        try:
            fig, ax, pvals, final_idx, final_labels = plot_selected_positions_boxplot_edge_based(
                ...
                p_threshold=0.05,
                figsize=(12, 5.8),
                title="%s flanking contribution for heart model" % motif_name,
                pdf_path="%s/%s_flank_edge_based_top1_bottom1_heart_fig2list.pdf" % (outdir, motif_name_safe),
                ylim=None,
                high_label="Top 1%",
                low_label="Bottom 1%",
            )
        ...
        try:
            ...
            result = plot_fig4b_like(
                ...
                figsize=(10, 8),
                title="%s flanking regions for heart model" % motif_name,
                pdf_path="%s/fig4b_like_%s_top_sorted_heart_fig2list.pdf" % (outdir, motif_name_safe),
                box_ylim=None,
                effect_norm="std",
    ...
    summary_csv_path = "%s/motif_processing_summary_threshold0p7.csv" % outdir
    ...
    if len(all_boxplot_pvals) > 0:
        ...
        combined_boxplot_pvals_path = "%s/all_motifs_boxplot_pvalues_threshold0p7.csv" % outdir
