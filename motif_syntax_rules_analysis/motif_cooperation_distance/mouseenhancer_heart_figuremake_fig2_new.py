...
#         tissue = "heart"
#         res_fwd = torch.load(
#             f"/users/ken.murakami/workspace/mouse_enhancer/fig2motif_heart/{motif1}_{motif2}_res_{tissue}.pt"
#         )
#         res_rc = torch.load(
#             f"/users/ken.murakami/workspace/mouse_enhancer/fig2motif_heart/{motif1}_{motif2}_res_rc_{tissue}.pt"
#         )
...
#         )
#         plt.tight_layout()
#         out_pdf = f"/users/ken.murakami/workspace/mouse_enhancer/fig2motif_heart/{motif1}_{motif2}_coop_by_dist_{tissue}_ver2.pdf"
...
if True:
    ...
    for i,motif1 in enumerate(motiflist):
        ...
        for j,motif2 in enumerate(motiflist):
            print("motif2:",j,motif2)
            res_fwd=torch.load(f"/users/ken.murakami/workspace/mouse_enhancer/fig2motif_heart/{motif1}_{motif2}_res_{tissue}.pt")
            res_rc=torch.load(f"/users/ken.murakami/workspace/mouse_enhancer/fig2motif_heart/{motif1}_{motif2}_res_rc_{tissue}.pt")
            coop_fwd=res_fwd["coop_by_dist"]
            coop_rc=res_rc["coop_by_dist"]
            ...
            plt.tight_layout()
            out_pdf=f"/users/ken.murakami/workspace/mouse_enhancer/fig2motif_heart/{motif1}_{motif2}_coop_by_dist_{tissue}_line_only_orientation.pdf"
            plt.savefig(out_pdf,bbox_inches="tight")
            plt.close()
            print(f"Saved to: {out_pdf}")
    orientation_hits_path=f"/users/ken.murakami/workspace/mouse_enhancer/fig2motif_heart/orientation_significant_pairs_{tissue}_absdist_ge10.txt"
...
if True:
    ...
    import matplotlib.pyplot as plt
    tissue="heart"
    base_dir="/users/ken.murakami/workspace/mouse_enhancer/fig2motif_heart"
    n=len(motiflist)
...
for i, motif1 in enumerate(motiflist):
    ...
    for j, motif2 in enumerate(motiflist):
        ...
        plt.tight_layout()
        out_pdf=f"/users/ken.murakami/workspace/mouse_enhancer/fig2motif_heart/{motif1}_{motif2}_absdist_median_IQR_{tissue}.pdf"
        plt.savefig(out_pdf,bbox_inches="tight")
        plt.close()
        print(f"Saved to: {out_pdf}")
