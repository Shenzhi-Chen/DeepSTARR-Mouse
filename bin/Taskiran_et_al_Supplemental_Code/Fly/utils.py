...
def plot_deepexplainer_givenax_withrc(explainer, fig, ntrack, track_no, seq_onehot, class_no):
    class_no = class_no - 1
    shap_values_, indexes_ = explainer.shap_values([seq_onehot,seq_onehot[:,::-1,::-1]],
                                                   output_rank_order=str(class_no),
                                                   ranked_outputs=1,
                                                   check_additivity=False)
    forward_ = shap_values_[0][0][0]*seq_onehot[0]
    reverse_ = (shap_values_[0][1][0]*seq_onehot[:,::-1,::-1][0])[::-1,::-1]
    _, ax1 = plot_weights((forward_+reverse_)/2,
                          fig, ntrack, 1, track_no,
                          title="Topic_" + str(class_no+1), subticks_frequency=10, ylab="DeepExplainer")
    return ax1


def plot_mutagenesis_givenax_fast_withrc(model, fig, ntrack, track_no, seq_onehot, class_no):
    ...
    ax = fig.add_subplot(ntrack, 1, track_no)
    ax.set_ylabel('In silico\nMutagenesis\nTopic_'+str(class_no+1))
    ax.set_title("Topic_" + str(class_no+1))
    ax.scatter(range(seq_shape[0]), -1*arr_a, label='A', color='green')
    ax.scatter(range(seq_shape[0]), -1*arr_c, label='C', color='blue')
    ax.scatter(range(seq_shape[0]), -1*arr_g, label='G', color='orange')
    ax.scatter(range(seq_shape[0]), -1*arr_t, label='T', color='red')
...
def insilico_evolution(regions, model, class_no, n_mutation, rc=False):
    ...
    for id_ in range(len(regions)):
        ...
        for i in range(n_mutation):
            ...
            if rc:
                prediction_mutagenesis_X = model.predict([mutagenesis_X["X"],mutagenesis_X["X"][:,::-1,::-1]])
                original_prediction = model.predict([start_x,start_x[:,::-1,::-1]])
            else:
                prediction_mutagenesis_X = model.predict(mutagenesis_X["X"])
...
def random_drift(regions, model, class_no, n_mutation, rc=False):
    #from scipy.stats import zscore
    nuc_to_onehot = {"A":[1, 0, 0, 0],"C":[0, 1, 0, 0],"G":[0, 0, 1, 0],"T":[0, 0, 0, 1]}
    mutation_pred = []
    mutation_loc = []
    ...
    for id_ in range(len(regions)):
        ...
        for i in range(n_mutation):
            ...
            if rc:
                ...
                #prediction_mutagenesis_X = model.predict(mutagenesis_X["X"])
                original_prediction = model.predict(start_x)
            next_one = mutagenesis_X["ids"][np.random.choice(1500,1)[0]]
            pred.append(original_prediction)
            mut.append(next_one)
            start_x[0][int(next_one.split("_")[0]),:] = np.array(nuc_to_onehot[next_one.split("_")[1]], dtype='int8')
        if rc:
...
def plot_deepexplainer_givenax(explainer, fig, ntrack, track_no, seq_onehot, class_no):
    class_no = class_no - 1
