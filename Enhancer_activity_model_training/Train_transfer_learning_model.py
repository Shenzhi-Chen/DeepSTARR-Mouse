...
def plot_density(X_test, Y_pred, Y_test):
    pred = main_model.predict(X_test, batch_size=params['batch_size'])
    data = {
        'predication_score': pred.flatten(),
        'enhancer_activity': np.array(Y_test)
    }
    df = pd.DataFrame(data)
    fig = plt.figure(figsize=(6, 4))
    sns.kdeplot(df, x="predication_score", hue="enhancer_activity", palette="cool", label="Category")
...
def prepare_input(fold, set, output, remove_no_ov=True, balanced_negative=True, keep_only_mm10_seq = False, filter_negative_sequence=False, n=5):
    ...
    if keep_only_mm10_seq:
        IDs = [True if 'mm' in s else False for s in Activity["ID"]]
        Activity = Activity[IDs]
    ...
    if filter_negative_sequence:
        IDs = (Activity["activity"] == "active")
        Activity = Activity[IDs]
...
# keep non-accessible regions
X_train_sequence, X_train_seq_matrix, X_train, Y_train = prepare_input(fold, "training", output, remove_no_ov=False, balanced_negative=False, keep_only_mm10_seq = False, filter_negative_sequence=False, n=10)
X_valid_sequence, X_valid_seq_matrix, X_valid, Y_valid = prepare_input(fold, "validation", output, remove_no_ov=False, balanced_negative=False, keep_only_mm10_seq = False, filter_negative_sequence=False, n=10)
X_test_sequence, X_test_seq_matrix, X_test, Y_test = prepare_input(fold, "test", output, remove_no_ov=False, balanced_negative=False, keep_only_mm10_seq = False, filter_negative_sequence=False, n=10)
